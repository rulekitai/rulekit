...
async function runAiSdk() {
  const { parseProfile } = await import("@rulekit/agent/profile")
  const { createRulesAgent } = await import("@rulekit/agent/runtime")
  const { SqliteStore } = await import("@rulekit/corpus/sqlite-store")
...
async function runEve() {
  const { decodeEvents } = await import("@rulekit/agent/events")
