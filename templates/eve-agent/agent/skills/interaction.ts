...
//
// The procedure itself lives once, in `@rulekit/agent/skills`, and both runtimes
// read it from there. Eve holds only the wiring, so a change to the procedure
// reaches this template with no edit here.
import { findSkill } from "@rulekit/agent/skills"
import { defineSkill } from "eve/skills"

const skill = findSkill("interaction")
if (!skill) throw new Error("the interaction skill is missing from @rulekit/agent/skills")
